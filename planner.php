            ...
            <div class="container card-container">
                <div class="card">
                    ...
                    <div id="popularPlans" class="mb-4">
                        <div class="mb-3">
                            <label for="populars" class="form-label fw-semibold">Select Origin</label>
                            <select id="populars" class="form-select" name="origin" list="all_places">
                                <?php
                        ...
                        <button type="button" onclick="search_plan_popular(this)" class="btn btn-primary w-100 mb-2">
                            <i class="fas fa-search me-2"></i>Check Plans
                        </button>
                        <button type="button" onclick="fill_manually(this)" class="btn btn-outline-secondary w-100">
                            <i class="fas fa-pen me-2"></i>Trip not included?
                        </button>
                    </div>
                    ...
                    <div id="tripPlannerForm" class="mb-4">
                        <div class="mb-3">
                            ...
                            <input id="origin" class="form-control" name="origin" list="all_places" autocomplete="off" />
                        </div>
                        <div class="mb-3">
                            <label for="destination" class="form-label fw-semibold">Destination</label>
                            <input id="destination" class="form-control" name="destination" list="all_places" autocomplete="off" />
                        </div>
                    ...
                    <div id="helloWorld" class="mt-4">
                        ...
                        <?php if ($logged): ?>
                        <button type="button" id="followBTN" onclick="follow_plan(this)" class="btn btn-success mt-3 w-100">
    ...
    <script>
        ...
        function search_plan_manually(ele) {
            ...
            if (origin && destination) {
                ...
                destination = destination ? destination.dataset.value : null;
                if(origin && destination) {
                  searchPlans(origin, destination);
                }
            }
        }
        ...
        function searchPlans(origin, destination) {
            fetch(`ajax/trekinfo.php?o=${origin}&d=${destination}`)
                .then(response => {
                    if (!response.ok) throw new Error('Network response was not ok');
                    return response.text();
                })
